...
- **系统管理员**：管理模型更新、拓展包上传、日志查看（仅离线版本可选）。
...
classDiagram
    ...
    class ResourceController {
        ...
        -string device
        -Map~string, np.array~ buffers
        +requestModel(id: string) %% 包含显存锁定逻辑
        +releaseAll()
        +getBuffer(name)
        +runPipeline(file: AudioFile) 
    }
    ...
    class AnalysisEngine {
        ...
        +start(file: AudioFile) AnalysisResult
    }
    ...
    class Separator { +run(audio: AudioFile) Map~stems~ }
    class RhythmAnalyzer { +run(stem: np.array) BeatMap }
    class ChordAnalyzer { +run(stem: np.array) ChordSequence }
    class FutureDLC { +run(context) }

    %% 关联关系
    AnalysisEngine *-- ResourceController
    AnalysisEngine o-- PluginManager
    
    %% 插件注册到管理器
    PluginManager o-- BasePlugin
    
    %% 具体实现继承
    BasePlugin <|-- Separator
    BasePlugin <|-- RhythmAnalyzer
    BasePlugin <|-- ChordAnalyzer
    BasePlugin <|-- FutureDLC

    %% 所有的插件直接向 RC 申请资源和读取数据
    BasePlugin ..> ResourceController : 申请显存/读取Buffer
```
...
flowchart TD
    ...
    subgraph ResourceController_Scope [ResourceController 资源与逻辑管控]
        direction TB
        R1[加载 RoFormer 并锁定显存] --> R2[执行 Separator]
        R2 --> R3[写入元数据并释放显存]
        
        R3 --> R4[加载乐理模型组]
        R4 --> R5[并行执行 Rhythm & Chord 插件]
        R5 --> R6[输出数据并释放所有资源]
    end

    U1 --> R1
    R6 --> D1[SQLite 存储与前端 Canvas 渲染]

    subgraph DLC_Scope [其他插件扩展]
        P1[用户选择 DLC 插件] --> P2[RC 提供对应 Stem Buffer]
        P2 --> P3[插件执行并更新数据]
    end
