...
flowchart TD
    ...
    subgraph Core_Analysis [显存串行调度]
        S1[RC: 加载 RoFormer]
        S1 --> S2[执行音轨分离]
        S2 --> S3[RC: 释放 RoFormer]
    ...
    D1 --> CheckPlugin{是否选择可用插件?}
    CheckPlugin -- 是 --> Plugin_Inference
    CheckPlugin -- 否 --> U3
