...
## 七、实现现状 vs 文档差距（2026-07-12 评估）

> **评估方法**：对照 §二 模块行为映射表，逐项标记实现状态。

### 7.1 已实现（✅）

| 用户行为 | 状态 |
|----------|------|
| 打开软件 → 文件系统扫盘 + 侧边栏 | ✅ 182 tests pass |
| 新建车间 → active + Tab1 | ✅ |
| 点历史车间 → `LastTab` 恢复 | ✅ |
| Tab1 本地上传 → `set_raw_audio_from_bytes` + 自动命名 | ✅ |
| Tab1 URL 上传 → yt-dlp 下载 + 进度条 + 视频标题命名 | ✅（B站除外） |
| Tab1 不自动跳转 → 用户点「继续」进 Tab2 | ✅ |
| Tab2 模型下拉 → `/api/plugins/separators` | ✅ |
| Tab2 开始分离 → SSE progress + done | ✅ Mock 通过 |
| SSE 事件推送 | ✅ `/api/events` |
| 车间 close / delete / switch | ✅ |
| 4 Tabs 导航 | ✅ |
...
| 用户行为 | 状态 | 缺口 |
|----------|------|------|
| Tab2 分离完成 → 显示 6 道音轨波形 | ⚠️ | 后端 SSE 推 `separation_done`，**前端未渲染** stem grid |
| Tab3 工具列表 | ⚠️ | `/api/plugins/analyzers` 返回 `example_analyzer`，**前端无 UI** |
| 可视化 JSON | ⚠️ | `/api/.../visualization` 有 mock 实现，**前端 Canvas 未接** |

### 7.3 未实现（❌）

| 用户行为 | 缺口 |
|----------|------|
| Tab2 分离模型多选 / 切换 | 前端下拉只有 1 个，不支持多模型注册 |
| Tab2 重跑覆盖 UX 提示 | §五-5，无实现 |
| Tab3 音轨选择 UI | **整个 Tab3 面板为空** |
| Tab3 分析工具选择 + 运行按钮 | **整个 Tab3 面板为空** |
| Tab3 分析进度 | `analysis_progress` 事件被推但前端无 handler |
| Tab3 分析完成展示 | `analysis_done` 被推但无 UI |
| Tab4 6 道波形渲染 | 前端无 Canvas 代码 |
| Tab4 混音控件 (M/S/Vol) | 无 UI |
| Tab4 播放 / 暂停 / seek | 无实现 |
| Tab4 MIDI 导出 | 无按钮 + stub 代码 |
| 6 轨混合 audio 端点 | `get_mix_audio()` 缺失 |
| 跨 Tab 跳转约束 | UI 不检查"必须先完成 Tab1 才能进 Tab2" |
| isStale / 过期提醒 | 无实现 |
| 错误弹窗 | 有 toast，但无模态框 |

### 7.4 仍待明确的用户行为（需团队拍板）

> 标注 P = 必须拍板的决策点，否则编码会卡。

| # | 行为 | 需要明确的细节 | 优先级 |
|---|------|---------------|--------|
| P1 | **URL 上传失败怎么办** | 是 toast + 回退到 Tab1，还是弹重试？ | 🔴 |
| P2 | **分离重跑时旧 stem 怎么处理** | 直接覆盖？弹确认框？保留旧版本？ | 🔴 |
| P3 | **分析任务重跑** | 同 track + 同 tool 的 task_id 复用（当前逻辑），还是每次新建 task_id 保留历史？ | 🔴 |
| P4 | **Tab3 用户选中 track 后，不选工具就点"开始"** | 拒绝？默认选第一个？ | 🟡 |
| P5 | **Tab3 多条 track 同时分析** | 允许？每条 track 独立按钮？还是"全部开始"？ | 🟡 |
| P6 | **Tab4 首次进入，无分析结果时显示什么** | 显示波形？显示"无分析结果"占位？ | 🟡 |
| P7 | **Tab4 用户调 M/S 后，播放中实时生效** | 前端 `<audio>` 直接响应？还是后端重推 mix audio？ | 🟡 |
| P8 | **Tab4 显示几条 stem** | 6 道全部显示，还是只显示用户分析过的那几条？ | 🟡 |
| P9 | **A-B loop UI** | Tab4 时间轴上拖 A 点 / B 点？输入框？ | 🟡 |
| P10 | **速度切换** | 下拉 0.5x-2x？还是滑块？ | 🟡 |
| P11 | **永久删除后用户反悔** | `keep_state` 备份已实现，但 UI 上怎么恢复？ | 🟢 |
| P12 | **i18n** | 中文还是英文？所有按钮目前英文 | 🟢 |
| P13 | **用户直接点 Tab2/Tab3/Tab4（跳过前序 Tab）** | 允许？如果要拒绝，显示什么提示？ | 🟡 |
| P14 | **车间名后期修改** | 侧边栏双击重命名？加"重命名"按钮？ | 🟢 |

### 7.5 建议的下一步（按可执行顺序）

| 顺序 | 任务 | 估时 |
|------|------|------|
| 1 | **开短会拍板 P1~P4**（4 个 🔴 决策） | 30min |
| 2 | **实现 Tab2 6 道 stem 展示**（SSE `separation_done` → 前端渲染） | 0.5d |
| 3 | **实现 Tab3 最小 UI**（track 列表 + tool 下拉 + 开始按钮） | 1d |
| 4 | **实现 Tab4 最小 UI**（6 道波形 + `<audio>` 播放） | 1d |
| 5 | **开短会拍板 P5~P13**（🟡 决策） | 30min |
| 6 | 实现 Tab3 分析流程全链路（真 plugin 调 AE） | 1d |
| 7 | 实现 Tab4 混音 + 播放 + seek | 1d |
| 8 | 实现 MIDI 导出 | 0.5d |
